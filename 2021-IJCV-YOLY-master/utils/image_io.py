...
def resize_to_multiple_of_32(tensor, multiple=32):
    B, C, H, W = tensor.shape

    # 计算新的高度和宽度
    new_size = (H - H % multiple,
                W - W % multiple)
...
#    print(img[0].size)
#    img_pil = img[0]
    img_pil = crop_image(img[0], d=32)  #这段代码的目的是对输入的图片进行居中裁剪，使得裁剪后的图片尺寸是 d 的整数倍，并且保持原始图片的中心部分。
...
def pil_to_np(img_PIL, with_transpose=True):
    ...
    """
    # 确保输入是PIL图像对象，如果不是则尝试转换
    if hasattr(img_PIL, 'mode'):  # 检查是否是PIL图像对象
        ar = np.array(img_PIL)
    else:
        # 如果已经是numpy数组，直接使用
        ar = img_PIL
